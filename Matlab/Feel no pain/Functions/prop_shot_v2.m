function [prob,tol]=prop_shot_v2(p_in,hp,dmg,fnp,n,nc,kills_in,prob,tol_mode,tol,p_all)
...
if nc < n %Still shots to do
    ...
    for i = 1:length(p_cur)
        ...
        if p_cur(i) > tol
            [prob,tol]=prop_shot_v2(p_cur(i),hp_c,dmg,fnp,n,nc,kills,prob,tol_mode,tol,p_all);
        end
    end
else %reached the last shot, fill in p
    prob(kills_in+1,hp) = prob(kills_in+1,hp) + p_in;
    if tol_mode < 0
        tol = max(tol,p_in/100000);
    end
end
